...
void display_results(Data_passer *data_passer) {
   ...
   // data_passer -> datatype_strings = datatype_strings;
   // g_hash_table_insert(pointer_passer, &KEY_DATA_TYPE_STRINGS, datatype_strings);

  //  GHashTable *field_analysis_hash = (GHashTable *)g_hash_table_lookup(pointer_passer, &KEY_FIELD_ANALYSIS_HASH);

  //  guint number_of_columns = get_number_of_columns(data_passer -> field_analysis_hash);
 ...
 */
 ...
 //   g_print("STEP 6\n");
 data_passer -> field_clause = g_strjoinv(", ", column_strings);
